...
#from __future__ import generators #-IMPORTNANNY
...
__license__ = '''###############################################################################
# The contents of this file are subject to the PyTis Public License 
...
salt = 'hippity hop hippity hop easter is on it's way!'
version = 1
language = English
supress_error_popups = False
...
def load(fname, write=False):
    ...
    Use ConfigObj to load a dictionary stored in a file.
    Try to forcefully create the file and directories it resides in
    if they are missing.
    
    :write:
    XXX - TODO, locking for writing and updating
    """
    ...
    try:
        ...
        import util as UTL
        if write:
            old_dir = os.path.abspath(os.path.dirname(fname))
            backup_path = os.path.abspath("%s.backup" % fname)
            backup_handle = open(backup_path, 'w')
            old_handle = file(fname, 'rb')

            backup_handle.write(old_handle.read(-1).replace("\r\n",'\n'))
            backup_handle.close()
            old_handle.close()
            os.remove(fpath)
            UTL.emergency_bug[0] = True
            return load(fpath, True)
        else:
            raise ParseError, e
    else:
        return config
